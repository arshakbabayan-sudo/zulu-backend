...
class TransferService
{
    ...
    public function listForCompanies(array $companyIds, array $filters = []): Collection
    {
        ...
        return $query->with(['offer', 'originLocation', 'destinationLocation', 'translations', 'offer.translations'])->get();
    }
    ...
    public function paginateForCompanies(array $companyIds, array $filters = [], int $perPage = 15): LengthAwarePaginator
    {
        ...
        return $query->with(['offer', 'originLocation', 'destinationLocation', 'translations', 'offer.translations'])->paginate($perPage);
    }
    ...
    public function findForCompanyScope(int|string $id, array $companyIds): ?Transfer
    {
        ...
        return $this->baseTenantTransferQuery($companyIds)
            ->whereKey($id)
            ->with(['offer', 'originLocation', 'destinationLocation', 'translations', 'offer.translations'])
            ->first();
    }
        ...
        if ($userEmail !== null) {
            ...
            $query->whereExists(function ($sub) use ($like): void {
                $sub->select(DB::raw(1))
                    ...
                    ->whereColumn('order_items.item_id', 'transfers.id')
                    ->where('order_items.item_type', 'transfer')
            ...
            $query->whereExists(function ($sub) use ($like): void {
                $sub->select(DB::raw(1))
                    ...
                    ->where(function ($q) use ($like): void {
                        $q->where('invoices.unique_booking_reference', 'like', $like)
                            ->orWhere('invoices.vendor_locator', 'like', $like);
                    });
            });
        }
    }
